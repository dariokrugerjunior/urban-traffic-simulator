...
import com.trafficsimulator.trafficstate.infrastructure.messaging.event.StreetClearedEvent;
...
public class KafkaStreetCongestionPublisher implements StreetCongestionPublisher {
    ...
    public void publishCleared(Street street) {
        StreetClearedEvent event = new StreetClearedEvent(
                street.id(), street.name(), street.congestionLevel().name(),
                street.congestionRatio(), Instant.now());
        kafkaTemplate.send(KafkaTopicsConfig.STREET_CLEARED, street.id(), event);
    }
}
